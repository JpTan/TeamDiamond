...
from account.models import Student, Loan, Payment
from django.views.generic import View
from .forms import PaymentForm


def index(request):
    return render(request, 'oas/index.html')


def view_applicants(request):
    applicant_list = Student.objects.filter(loan__status="In Process")

    return render(request, 'oas/applicants.html', {'applicant_list': applicant_list})
...
def view_applicant_detail(request, idnum):
    ...
    return render(request, 'oas/app_details.html', {'student': student, 'loan': loan})
...
def view_loaners(request):
    ...
    return render(request, 'oas/loaners.html', {'loaners': loaners})


def view_loaner_detail(request, idnum):
    student = Student.objects.get(id_number=idnum)
    loan = Loan.objects.get(id_number=student)
    return render(request, 'oas/loaner_details.html', {'student': student, 'loan': loan})


def view_payments_pending(request):
    payments = Payment.objects.filter(isApproved=False)
    show_all = False
    return render(request, 'oas/payments.html', {'payments': payments, 'show_all': show_all})
...
def view_payment_detail(request, pk):
    ...
    student = Student.objects.get(id_number=payment.idNum.idNum)
    loan = Loan.objects.get(id_number=student)
    return render(request, 'oas/payment_details.html', {'student': student, 'payment': payment, 'loan': loan})
...
def edit_payment(request, pk):
    ...
    return render(request, 'oas/payment_edit.html', {'form': form, 'student': student, 'payment': payment, 'loan': loan})
